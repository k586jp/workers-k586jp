...
export function PageLayout(props: Article[], nonce: string) {
    ...
    if (count === 0) {
        ...
        for (let i = 0; i < count; i++) {
            articlesHtml += `<li><a href="/article/${props[i].id}">[<time datetime="${props[i].created_at}">----/--/-- --:--</time>] ${props[i].title}</a></li>`;
        }
        articlesHtml += '</ul></div>';
    } else {
        for (let i = 0; i < count; i++) {
            articlesHtml += `<div><h6><time datetime="${props[i].created_at}">----/--/-- --:--</time></h6><h1><a href="/article/${props[i].id}">${props[i].title}</a></h1>${props[i].content_html}</div>`;
        }
    }
